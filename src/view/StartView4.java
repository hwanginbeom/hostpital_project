...
public class StartView4 {
   public static void main(String[] args) {
      ...
      while (result != 10) {
         ...
         System.out.println("  4 - 해당 지역  병원 수 검색");
         System.out.println("  5 - 해당 지역  의료인력 수 검색");
         System.out.println("  6 - 지역 별 병원 및 의료인력 수 검색 ");
         ...
         case 4:
            ...
            while (result != 0) {
               System.out.println("*** 해당 지역 별 병원을 보시겠습니까? 1(yes) /0(no) ");
               try {
               result = scan.nextInt();
               } catch (Exception e) {
                  System.out.println("정확한 값을 입력하세요");
               ...
               try {
                  for (int i = 0; i < Hospital_TotalDAO.getAll().size(); i++) {
                     System.out.println((i + 1) + " " + Hospital_TotalDAO.getAll().get(i).getLoc());
                  }
                  while (result != 0) {
